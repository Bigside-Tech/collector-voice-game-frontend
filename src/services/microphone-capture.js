import { AudioCaptureStreamingService, AudioCaptureService } from "./audio-service";

let websocketSendUrl, websocketReceiveUrl, jwtToken, websocket;
      ...
      grammars: [
        {
          ...
          public: {
            root:
              "<go> {go} | <stop> {stop} | <left> {left} | <right> {right} | <up> {up} | <down> {down}",
          },
          rules: {
            go:
              "(go)",
...
const connectWebsocket = async (onShowLoader, onStartCapture, onMessage) => {
      ...
      const fetchWebsocketUrl = async () => {
        ...
        try {
          ...
          if (fetchWebsocketResponse.ok) {
            ...
            startMicrophoneCapture(websocketSendUrl, websocketReceiveUrl, onStartCapture, onMessage);
          }
        } catch (err) {
...
const startMicrophoneCapture = (websocketSendUrl, websocketReceiveUrl, onStartCapture, onMessage) => {
          ...
          try {
            if (message.lastEvent === "RECOGNITION-COMPLETE") {
              ...
              if (message.status == "MATCH") {
                onMessage(message.alternatives[0].utterance);
            ...
            //   div.style.padding = "1rem";
              onMessage("ERROR-GETTING-RESULTS");
...
const stopMicrophoneCapture = (onStopCapture) => {
